...
def hexdump(src, length=16):
    ...
    for i in range(0, len(src), length):
        s = src[i:i + length].decode()
        hexa = ' '.join(["%0*X" % (digits, ord(x)) for x in s])
        text = ''.join([x if 0x20 <= ord(x) < 0x7F else '.' for x in s])
        result.append("%04X   %-*s   %s" % (i, 16 * (2 + 1), hexa, text))
    print('\n'.join(result))
...
def proxy_handler(client_socket,remote_host,remote_port,receive_first):
    ...
    if receive_first:
        ...
        # send it to oour response handler
...
def main():
    ...
    if len(sys.argv[1:]) != 5:
        print(f"""
              Usage: python3 ./{sys.argv[0]} [localhost] [localport] [remotehost] [remoteport] [receive_first]
              Example: python3 ./{sys.argv[0]} 127.0.0.1 9000 10.12.132.1 9000 True
              """)
        sys.exit(0)
